...
async def get_result(session_id: str):
    ...
    try:
        ...
        if hasattr(app.state, 'results') and session_id in app.state.results:
            ...
            # Find downloadable output files. Slides now produce editable PPTX,
            # while posters may still produce PDF or image outputs.
            pptx_file = next((f for f in output_files if f['filename'].lower().endswith('.pptx')), None)
            ...
            # Add download links
            if output_type == "slides" and pptx_file:
                response_data["ppt_url"] = f"/outputs/{pptx_file['relative_path']}"
            elif pdf_file:
                if output_type == "slides":
