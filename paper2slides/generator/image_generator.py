...
class ImageGenerator:
    ...
    def __init__(
        ...
        elif self.provider in ("openai", "openai_images", "openai-compatible", "openai_compatible"):
            self.client = None
    ...
    def _call_model(self, prompt: str, reference_images: List[dict]) -> tuple:
        ...
        if self.provider in ("openai", "openai_images", "openai-compatible", "openai_compatible"):
            return self._call_model_openai_images(prompt, reference_images)
        return self._call_model_openrouter(prompt, reference_images)

    def _call_model_openai_images(self, prompt: str, reference_images: List[dict]) -> tuple:
        ...
        # The images/generations API does not reliably accept reference images
        # across providers, so we fold lightweight figure context into the prompt.
        if reference_images:
            references = []
            for img in reference_images:
                fig_id = img.get("figure_id", "Figure")
                caption = (img.get("caption") or "").strip()
                references.append(f"{fig_id}: {caption}" if caption else fig_id)
            payload["prompt"] = (
                f"{prompt}\n\nReference figures to preserve concept consistency:\n- "
                + "\n- ".join(references)
            )
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"Calling OpenAI-compatible image API (attempt {attempt + 1}/{max_retries})...")
                ...
                if response.status_code >= 400:
                    logger.warning(f"Image API error {response.status_code}: {response.text[:300]}")
                ...
                if not b64_data:
                    raise RuntimeError(f"Image API returned no b64_json payload: {item}")

                mime_type = item.get("mime_type", "image/png")
                logger.info("Image generation successful (OpenAI-compatible images API)")
                return base64.b64decode(b64_data), mime_type
            except Exception as e:
                logger.error(f"Error in OpenAI-compatible image API call (attempt {attempt + 1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay * (attempt + 1))
                    continue
                raise
